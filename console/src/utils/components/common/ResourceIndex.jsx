...
export const ResourceIndex = ({ options }) => {
    ...
    const [searchQuery, setSearchQuery] = useSessionStorage(`${config.name}_search_query`, '')
    const [debouncedQuery, setDebouncedQuery] = useState(searchQuery)

    // ソート機能の状態を追加
    const [sortConfig, setSortConfig] = useSessionStorage(`${config.name}_sort_config`, {
        column: null,
        direction: 'asc',
    })
    ...
    const [advancedValues, setAdvancedValues] = useSessionStorage(
        `${config.name}_advanced_criteria_values`,
        {}
    )
    const [appliedAdvancedValues, setAppliedAdvancedValues] = useSessionStorage(
        `${config.name}_advanced_criteria_applied`,
        {}
    )
